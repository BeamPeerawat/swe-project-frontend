...
export default {
  ...
  methods: {
    ...
    async fetchDrafts() {
      try {
        const response = await this.$axios.get(`/api/opencourserequests/drafts?userId=${this.user._id}`, {
    ...
    async loadDraft(draftId) {
      try {
        const response = await this.$axios.get(`/api/opencourserequests/${draftId}?userId=${this.user._id}`, {
          withCredentials: true,
        });
    ...
    async deleteDraft(draftId) {
      if (confirm('ยืนยันการลบแบบร่างนี้?')) {
        try {
          await this.$axios.delete(`/api/opencourserequests/${draftId}?userId=${this.user._id}`, {
            withCredentials: true,
          });
          await this.fetchDrafts();
    ...
    async submitForm() {
      ...
      try {
        const response = await this.$axios.post('/api/opencourserequests/submit', {
          ...this.form,
    ...
    async saveDraft() {
      ...
      try {
        const response = await this.$axios.post('/api/opencourserequests/draft', {
          ...this.form,
          userId: this.user?._id,
          status: 'draft',
        }, {
